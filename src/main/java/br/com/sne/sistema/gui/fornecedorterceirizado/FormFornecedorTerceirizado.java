...
public class FormFornecedorTerceirizado extends DefaultForm<FornecedorTerceirizado, FornecedorTerceirizadoTableModel> {
	...
	public FormFornecedorTerceirizado() {
		super(new FornecedorTerceirizadoTableModel(), "/images/icon_fterceirizado_18.png", "FornecedorTerceirizadoes");
	}
	
	public boolean save(FornecedorTerceirizado current) {
		boolean s = false;
		try {
			Facade.getInstance().salvarFornecedorTerceirizado(current);
			fornecedorTerceirizado = current;
			
			s = true;
		} 
		catch (DuplicatedRegisterException err) {
			JOptionPane.showMessageDialog(this, "O Fornecedor Terceirizado já se encontra cadastrado", "ERRO", JOptionPane.ERROR_MESSAGE);
		}
	...
	public boolean update(FornecedorTerceirizado current) {
		boolean s = true;
		Facade.getInstance().atualizarFornecedorTerceirizado(current);
		return s;
	}
	
	public boolean remove(FornecedorTerceirizado current) {
		boolean test = false;
		try {
			Facade.getInstance().removerFornecedorTerceirizado(current);
			test = true;
		} catch (RuntimeException err) {
			err.printStackTrace();
			JOptionPane.showMessageDialog(this, "Erro ao remover o FornecedorTerceirizado.\nVerifique se existem Registros de Sublocação pendentes deste FornecedorTerceirizado antes de tentar removê-lo.", "ERRO", JOptionPane.ERROR_MESSAGE);
		}
		return test;
	}
	...
	public FornecedorTerceirizado newElement() {
		return new FornecedorTerceirizado();
	}

	public void loadInputFields(FornecedorTerceirizado fornecedorTerceirizado) {
		...
		fornecedorTerceirizado.setDadosBancarios2(dadosBancarios2);
		
		fornecedorTerceirizado.setDadosBancarios1(dadosBancarios1);
		
		fornecedorTerceirizado.setCodigo(fieldCodigo.getText());
		fornecedorTerceirizado.setNome(nome);
		fornecedorTerceirizado.setCnpj(cnpj);
		fornecedorTerceirizado.setTipo(tipo);
		fornecedorTerceirizado.setEmail(email);
		fornecedorTerceirizado.setFone(telefone);
		fornecedorTerceirizado.setRamal(ramal);
		fornecedorTerceirizado.setContato(contato);
		fornecedorTerceirizado.setCelular(celular);
		fornecedorTerceirizado.setObservacoes(observacoes);
		...
		fornecedorTerceirizado.setEndereco(endereco);
	}
	...
	protected void clear() {
		...
		fornecedorTerceirizado = null;
		fieldCodigo.setText("00");
	}

	protected void loadForm(FornecedorTerceirizado cli) {
		clear();
		if(cli.getTipo() != null) {
	...
	protected boolean validateFormInsert() {
		...
		if(fieldNome.getText().length() <= 0) {
			test = false;
			error += "\nPreencha o nome do Fornecedor Terceirizado";
		}
		if(Integer.parseInt(fieldCodigo.getText()) <=0) {
			test=false;
			error += "\nPreencha o código do fornecedor Terceirizado";
...
/*		if(fieldBanco1.getText().length() <=0) {
	...
	public List<FornecedorTerceirizado> listAll() {
		return Facade.getInstance().listarFornecedorTerceirizadoes();
	}

	@Override
	public boolean print(FornecedorTerceirizado current) {
		HashMap<String, Object> hm = new HashMap<String, Object>();

		if(fornecedorTerceirizado != null) {
			hm.put("id", fornecedorTerceirizado.getId());
			...
			try {
				...
				JasperPrint impressao = JasperFillManager.fillReport("br/com/sne/sistema/report/fornecedorTerceirizado.jasper", hm, c);
			...
			JOptionPane.showMessageDialog(this, "Selecione um Fornecedor Terceirizado para poder visualizar sua impressão", "ERRO", JOptionPane.ERROR_MESSAGE);
		}
		return true;
	}
}
