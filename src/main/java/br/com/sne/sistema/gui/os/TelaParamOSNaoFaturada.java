...
public class TelaParamOSNaoFaturada extends JFrame implements ActionListener{
	...
	public TelaParamOSNaoFaturada(){
		
		//configurando a janela
		setTitle("Relatório de Ordens de NÃO Faturadas por Período");
	...
	public void actionPerformed(ActionEvent e) {
		if(e.getSource() == botaoGerar) {
			...
			if((codVendedor != 0 && radioPorVendedor.isSelected()) || radioTodos.isSelected()){
				...
				if(radioTodos.isSelected()){
					arquivo = getClass().getResource("/br/com/sne/sistema/report/ordemServicoNaoFaturada.jasper"); 
				}
				else{
					hm.put("id", codVendedor);
					arquivo = getClass().getResource("/br/com/sne/sistema/report/ordemServicoNaoFaturadaIndividual.jasper"); 
				}
				try {
					setVisible(false);
		...
		if(e.getSource() == botaoLocalizarVendedor){
			...
			DialogSearchFuncionario teste = new DialogSearchFuncionario(TelaParamOSNaoFaturada.this, listaFuncionario);
			Funcionario f = teste.showDialog(TelaParamOSNaoFaturada.this);
			if(f != null) {
