...
public abstract class DefaultFormSemBotao<T, TModel extends SizedTableModel<T>> extends JInternalFrame  {
	...
	public DefaultFormSemBotao(TModel tableModel, String icon, String title) {
		super();
														...
														for(int i=0; i<numRow;i++) {
															for(int j=0;j<numCol;j++) {
																if(jTable.getValueAt(i, j)!=null)
																	buffer.append(jTable.getValueAt(i, j));
																if (j<numCol-1)
																	buffer.append("\t");
															}
															buffer.append("\n");
	...
	protected JTabbedPane getJTabbedPane() {
		if (jTabbedPane == null) {
			...
			jTabbedPane.addTab("Relatório", new ImageIcon(getClass().getResource("/images/lista_12.png")), getPainelRelatorio(), null);
