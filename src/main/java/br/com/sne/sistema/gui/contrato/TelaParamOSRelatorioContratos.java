...
public class TelaParamOSRelatorioContratos extends JFrame implements ActionListener{
	...
	public TelaParamOSRelatorioContratos(){
		
		//configurando a janela
		setTitle("Relatório de Contratos por Período");
	...
	public void actionPerformed(ActionEvent e) {
		if(e.getSource() == botaoGerar) {
			...
			if((codVendedor != 0 && radioPorVendedor.isSelected()) || radioTodos.isSelected()){
				...
				if(radioTodos.isSelected()){
					arquivo = getClass().getResource("/br/com/nordesti/locav/report/contratosPorPeriodo.jasper"); 
				}
				else{
					hm.put("id", codVendedor);
					arquivo = getClass().getResource("/br/com/nordesti/locav/report/contratosPorPeriodoIndividual.jasper"); 
				}
				try {
					setVisible(false);
		...
		if(e.getSource() == botaoLocalizarVendedor){
			...
			DialogSearchFuncionario teste = new DialogSearchFuncionario(TelaParamOSRelatorioContratos.this, listaFuncionario);
			Funcionario f = teste.showDialog(TelaParamOSRelatorioContratos.this);
			if(f != null) {
