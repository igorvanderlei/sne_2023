...
public abstract class DefaultGarbageForm<T, TModel extends SizedTableModel<T>> extends JInternalFrame {
	...
	private JPopupMenu menuRestaurar;
	
	public DefaultGarbageForm(TModel tableModel, String icon, String title) {
		super();
	...
	private JPanel getJContentPane() {
		if (jContentPane == null) {
			...
			menuRestaurar = new JPopupMenu();
			menuRestaurar.add(getMenuRestaurar());
		}
		return jContentPane;
	}

	private JMenuItem getMenuRestaurar() {
		JMenuItem mnu = new JMenuItem("Restaurar Registro");
		mnu.setIcon(new ImageIcon(getClass().getResource("/images/restore.png")));
		mnu.addActionListener(new ActionListener() {
			@SuppressWarnings("unchecked")
			public void actionPerformed(ActionEvent arg0) {
				if(jTable.getSelectedRowCount() > 0) {
					T current = (T) tableModel.getValueAt(jTable.getSelectedRow(), tableModel.getObjectIndex());
					restore(current);
					JOptionPane.showMessageDialog(null,"Registro restaurado com sucesso!");
					carregarTabela();
				}
			}
		});
						...
						public void mouseClicked(MouseEvent arg0) {
							if(arg0.getButton() == MouseEvent.BUTTON3) {
								if(jTable.getSelectedRow() >= 0 )
									menuRestaurar.show(jTable, arg0.getX(), arg0.getY());
							} 						
						}
					}
	...
	public abstract boolean restore(T current);
