...
public class TelaBloquear extends JDialog {
	...
	private void initialize() {
		...
		if (Facade.getInstance().getUsuarioLogado().getLogin() == null) {
			setVisible(false);
			return;
		}
	...
	private JPanel getJContentPane() {
		if (jContentPane == null) {
			...
			jContentPane.add(getJPanelDadosCadastraisEndereco(),
					getJPanelDadosCadastraisEndereco().getName());
		}
		return jContentPane;
	}
	...
	private JPanel getJPanelDadosCadastraisEndereco() {
		if (jPanelDadosCadastraisEndereco == null) {
			...
			jLabel.setIcon(new ImageIcon(getClass().getResource("/images/login3.jpg")));
			...
			jPanelLogin
					.setBorder(BorderFactory.createCompoundBorder(BorderFactory
							.createTitledBorder(BorderFactory
									...
									"Usuário", TitledBorder.LEFT,
									TitledBorder.DEFAULT_POSITION, new Font(
											"Arial", Font.BOLD, 12), new Color(
											51, 51, 51)), BorderFactory
	...
	private JTextField getJTextField() {
		if (fieldLogin == null) {
			...
			fieldLogin.setText(Facade.getInstance().getUsuarioLogado()
					.getLogin());
	...
	private JPanel getJPanelbuttonOK() {
		if (jPanelbuttonOK == null) {
			...
			jPanelbuttonOK.setSize(new Dimension(300, 60));
	...
	private void logar() {
		...
		if (logado.verificarPassword(password)) {
			fieldLogin.setText("");
