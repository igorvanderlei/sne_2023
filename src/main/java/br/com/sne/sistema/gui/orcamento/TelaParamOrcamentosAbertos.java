...
public class TelaParamOrcamentosAbertos extends JFrame implements ActionListener{
	...
	public TelaParamOrcamentosAbertos(){
		
		//configurando a janela
		setTitle("Relatório de Orçamentos Abertos por Período");
	...
	public void actionPerformed(ActionEvent e) {
		if(e.getSource() == botaoGerar) {
			...
			if((codVendedor != 0 && radioPorVendedor.isSelected()) || radioTodos.isSelected()){
				...
				if(radioTodos.isSelected()){
					arquivo = getClass().getResource("/br/com/sne/sistema/report/orcamentosSemOS.jasper"); 
				}
				else{
					hm.put("id", codVendedor);
					arquivo = getClass().getResource("/br/com/sne/sistema/report/orcamentosSemOS2Individual.jasper"); 
				}
				try {
					setVisible(false);
		...
		if(e.getSource() == botaoLocalizarVendedor){
			...
			DialogSearchFuncionario teste = new DialogSearchFuncionario(TelaParamOrcamentosAbertos.this, listaFuncionario);
			Funcionario f = teste.showDialog(TelaParamOrcamentosAbertos.this);
			if(f != null) {
