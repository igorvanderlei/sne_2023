...
import br.com.sne.sistema.gui.util.form.DialogSearchOrcamento;
import br.com.sne.sistema.gui.util.form.DialogSearchOrcamento3;
...
public class FormOrdemServicoExtra extends DefaultForm<OrdemServico, OrdemServicoTableModel> implements FormIntervalar, Nota {
	...
	private JCheckBox fieldLocalSalvar;
	...
	public FormOrdemServicoExtra() {
		super(new OrdemServicoTableModel(), "/images/produto20.png", "Ordem de Serviço Extra");
		clear();
	}
	...
	public boolean save(OrdemServico current) {
		...
		try {
			...
			boolean salvarLocal = fieldLocalSalvar.isSelected(); 
			if(ordemServ != null) {
		...
		//	Facade.getInstance().atualizarOrdemServico(current);
			...
			if(salvarLocal) {
				Facade.getInstance().salvarLocal(new Local(current.getLocal()));
			}
	...
	public boolean update(OrdemServico current) {
		...
		if(agencia != null) {
			...
			if(current.getTotalAgencia() == null) {
				current.setTotalAgencia(calcularComissao());
				current.setDadosAgencia(agencia);
				current.setPercentualAgencia(new Integer(fieldPercentualComissao.getText()));
				current.setVencimentoAgencia(fieldDataVencimento.getDate());
		...
		if(comissao.getValor() != null) {
			BigDecimal valorComissaoAntiga = comissao.getValor();
			if(!comissao.isSituacao()) {
				current.setTotalAgencia(calcularComissao());
				Facade.getInstance().atualizarDespesa(comissao);
			}
			else if(!valorComissaoAntiga.equals(calcularComissao())){
...
/*	private JPanel getPanelFaturamento() {
		...
		botaoExtornar .addActionListener(
				new ActionListener() {
					public void actionPerformed(ActionEvent arg0) {
						if(ordemServ==null) {
							JOptionPane.showMessageDialog(FormOrdemServicoExtra.this, "Selecione uma Ordem de Serviço", "ERRO", JOptionPane.ERROR_MESSAGE);
							return;
						}
						...
						if(possuiEquipamentoPendente){
							JOptionPane.showMessageDialog(FormOrdemServicoExtra.this, "Não é possível estornar a Ordem de Serviço pois existem equipamentos pendentes.", "ERRO", JOptionPane.ERROR_MESSAGE);
							return;
						}
						...
						if(possuiParcelaPaga) {
							JOptionPane.showMessageDialog(FormOrdemServicoExtra.this, "Não é possível estornar a Ordem de Serviço pois existem parcelas ou despesas pagas.", "ERRO", JOptionPane.ERROR_MESSAGE);
							return;
						}
						...
						if(motivo != null && motivo.length() > 3) {
							...
							JOptionPane.showMessageDialog(FormOrdemServicoExtra.this, "Informe o motivo.\nA Ordem de Serviço não foi estornada.", "ERRO", JOptionPane.ERROR_MESSAGE);
	...
	private JPanel getPanelEndereco () {
		...
		fieldLocalSalvar = new JCheckBox("Armazenar este local para utilização em outras ordens de serviço");
		...
		endereco.add(new TitledPanel("Ambientes do Evento(Auditório, Teatro, Sala, etc...)", getPanelAmbientes()), new RestricaoLayout(3,0,6,2, true, true));
		
		endereco.add(new TitledPanel("Observações do Local do Evento", scrollObservacoes), new RestricaoLayout(5,0,6,1, true, true));

		endereco.add(fieldLocalSalvar , new RestricaoLayout(6,0, 6, true, false));
		return endereco;
	}
...
JPanel panelAmbiente = new JPanel();
				...
				if(tabelaAmbiente.getSelectedRow() != -1  && ! tabelaAmbiente.isEditing()) {
					...
					if(ps != null) {
						...
						for(RecursoSolicitado rs : ps.getRecursosSolicitados()) {
							if(rs.getDataFim().compareTo(dataFinalMudanca) > 0) {
								rs.setDataFim(dataFinalMudanca);
							}
							if(rs.getDataInicio().compareTo(dataInicialMudanca) < 0) {
								rs.setDataInicio(dataInicialMudanca);
							}
		...
		tabelaAmbiente.getColumnModel().getColumn(1).setCellEditor(new JDateChooserCellEditorUnbounded());
		tabelaAmbiente.getColumnModel().getColumn(2).setCellEditor(new JDateChooserCellEditorUnbounded());
		...
		botaoAdicionarAmbiente.addActionListener(new ActionListener(){
			public void actionPerformed(ActionEvent e) {
				...
				if(fieldAmbientesEvento.getSelectedIndex() != 0) {
					AmbienteEvento amb;
					if(fieldAmbienteNome.getText() != null) {
						amb = new AmbienteEvento(fieldAmbienteNome.getText(), fieldAmbienteDataInicio.getDate(), fieldAmbienteDataFim.getDate());

					}
					else {
						amb = new AmbienteEvento(((SalaLocal)fieldAmbientesEvento.getSelectedItem()).getNome(), fieldAmbienteDataInicio.getDate(), fieldAmbienteDataFim.getDate());
					}					adicionarAmbiente(amb);
					adicionarAmbienteLogistica();
					adicionarAmbienteTerceirizado();
					limparAmbiente();
				}
				else if(paineisEquipamento.get(fieldAmbienteNome.getText()) == null) {
	...
	private JPanel getPanelPlanilha() {
		...
		ComponentKeyBinding.install(pdfController, viewerComponentPanel);
		...
		*/
		...
		//controller.openDocument(exemplo.buffer, 0, exemplo.buffer.length, "teste", null);
	...
	//	planilha.add(getBotaoCarregarPlanilha(), new RestricaoLayout(0,0,1, 1,true, true));
	...
	/*	private JButton getBotaoCarregarPlanilha() {
		buttonCarregarPlanilha = new JButton("Carregar Planilha ...", new ImageIcon(getClass().getResource("/images/desc_18.png")));
		...
		buttonCarregarPlanilha.addActionListener(
				new ActionListener() {
					public void actionPerformed(ActionEvent e) {
				        ...
				        int returnVal = fc.showOpenDialog(FormOrdemServicoExtra.this);
				        if (returnVal == JFileChooser.APPROVE_OPTION) {
				        	File file = fc.getSelectedFile();
				        	FormOrdemServicoExtra.this.abrirPlanilha(OrdemServico.converterPlanilhaString(file));
	...
	private JPanel getPanelComissao() {
		...
		panelComissao.add(new JPanel(), new RestricaoLayout(3,0,6,1,true,true));
	...
	public JButton getBotaoProcurarAgencia() {
		...
		botaoProcurarAgencia.addActionListener(
				new ActionListener() {
					public void actionPerformed(ActionEvent arg0) {
						DialogSearchFornecedor teste = new DialogSearchFornecedor(FormOrdemServicoExtra.this);
						Fornecedor c = teste.showDialog(FormOrdemServicoExtra.this);
						if(c != null)
							carregarAgencia(c);
					}
				}	
					...
					public void actionPerformed(ActionEvent e) {
						...
						if(ordemServ != null) {
							if(ordemServ.getStatus() == StatusOS.ESTORNADA) {
								JOptionPane.showMessageDialog(FormOrdemServicoExtra.this, "Não é possível alterar uma Ordem de Serviço Estornada.", "ERRO", JOptionPane.ERROR_MESSAGE);
								return;
							}
							
							if(rem.isSituacao()) {
								JOptionPane.showMessageDialog(FormOrdemServicoExtra.this, "Não é possível remover uma parcela paga", "ERRO", JOptionPane.ERROR_MESSAGE);
								return;
							}
							if(rem.getId() != null)
	...
	private JButton getBotaoAdicionarParcela() {
		...
		botaoAdicionarParcela.addActionListener(new ActionListener() {
			...
			/*	if(ordemServ==null) {
				...
				if(ordemServ.getStatus() == StatusOS.ESTORNADA) {
					JOptionPane.showMessageDialog(FormOrdemServicoExtra.this, "Não é possível adicionar parcelas em uma Ordem de Serviço Estornada.", "ERRO", JOptionPane.ERROR_MESSAGE);
					return;
				}
				BigDecimal valor= new BigDecimal(0);
				try {
					if(fieldValorParcela.getValor() == null) {
						JOptionPane.showMessageDialog(FormOrdemServicoExtra.this, "Informe o valor da parcela", "ERRO", JOptionPane.ERROR_MESSAGE);
						return;
					} else {
				...
				if(fieldDataParcela.getDate() == null) {
					JOptionPane.showMessageDialog(FormOrdemServicoExtra.this, "Informe a data da parcela!", "ERRO", JOptionPane.ERROR_MESSAGE);
					return;
				}
		...
		botaoGerarParcelas.addActionListener(new ActionListener() {
			...
			/*	if(ordemServ==null) {
				...
				if(fieldQuantidadeParcela.getText().equals("") || Integer.parseInt(fieldQuantidadeParcela.getText()) <= 0) {
					JOptionPane.showMessageDialog(FormOrdemServicoExtra.this, "Informe a quantidade de parcelas!", "ERRO", JOptionPane.ERROR_MESSAGE);
					return;
				}
				if(fieldDataPrimeiraParcela.getDate() == null) {
					JOptionPane.showMessageDialog(FormOrdemServicoExtra.this, "Informe a data da primeira parcela!", "ERRO", JOptionPane.ERROR_MESSAGE);
					return;
				}
				...
				for(int i=1; i <= quantidadeParcela; i++) {
					...
					rec.setDescricao("Parcela " + i + " de " +  quantidadeParcela + " do evento " + FormOrdemServicoExtra.this.getNomeEvento());
					rec.setFuncionarioCadastro(Facade.getInstance().getUsuarioLogado().getFuncionario());
					rec.setOrdemServico(ordemServ);
	...
	private void adicionarAmbiente(AmbienteEvento amb) {
		if(paineisLogistica.get(amb.getNome()) == null && paineisTerceirizados.get(amb.getNome()) == null ) {
			PanelSala pn = new PanelSala(amb, FormOrdemServicoExtra.this, true, true,TipoRecurso.EQUIPAMENTO);
			PanelSala pnc = new PanelSala(amb, FormOrdemServicoExtra.this, false, false,TipoRecurso.CENOGRAFIA);
			PanelSala pnet = new PanelSala(amb, FormOrdemServicoExtra.this, false, false,TipoRecurso.EQUIPE_TECNICA);
	...
	private void adicionarAmbienteLogistica(AmbienteEvento amb) {
		PanelSala pnl = new PanelSala(amb, FormOrdemServicoExtra.this, false, false,TipoRecurso.LOGISTICA);
	...
	private void adicionarAmbienteTerceirizado(AmbienteEvento amb) {
		PanelSalaTerceirizado pnl = new PanelSalaTerceirizado(amb, FormOrdemServicoExtra.this, false, false,fieldTerceirizadoEmp.isSelected());
	...
	private void adicionarAmbienteTerceirizado() {
		if(paineisTerceirizados.size() == 0) {
			AmbienteEvento amb = new AmbienteEvento("TERCEIRIZADOS",fieldDataMontagem.getDate(),fieldDataFim.getDate());
...
/*	private JButton getBotaoImportarEnderecoCliente() {
	...
	private JPanel getPanelDadosEvento() {
		...
		fieldNomeContatoEvento.setEditable(false);
		fieldTelefoneContatoEvento.setEditable(false);
				...
				new PropertyChangeListener() {
					public void propertyChange(PropertyChangeEvent arg0) {
						if(fieldDataMontagem.getDate() != null) {
							...
							//fieldLogisticaDataInicio.setMinSelectableDate(fieldDataMontagem.getDate());
							//fieldLogisticaDataFim.setMinSelectableDate(fieldDataMontagem.getDate());
						}
				...
				new PropertyChangeListener() {
					public void propertyChange(PropertyChangeEvent arg0) {
						if(fieldDataInicio.getDate() != null) {
							...
							//fieldCenografiaDataFim.setMinSelectableDate(fieldDataInicio.getDate());
							
							
							//Long limiteMontagem = fieldDataInicio.getDate().getTime() - 7200000;
							fieldDataMontagem.setMaxSelectableDate(fieldDataInicio.getDate());
				...
				new PropertyChangeListener() {
					public void propertyChange(PropertyChangeEvent arg0) {
						if(fieldDataFim.getDate() != null) {
							fieldDataInicio.setMaxSelectableDate(fieldDataFim.getDate());
		...
		painelDadosEvento.add(pplanilha, new RestricaoLayout(3,0,8,1, true, true));
	...
	//	painelDadosEvento.add(new TitledPanel("Detalhes do Evento", panelDetalhes), new RestricaoLayout(3, 0,2,1, true, true));
	//	painelDadosEvento.add(new TitledPanel("Planilha", getPanelPlanilha()), new RestricaoLayout(3,2,6,1, true,true));
		...
		addLogisticaButton.addActionListener(new java.awt.event.ActionListener() {
			public void actionPerformed(java.awt.event.ActionEvent e) {
				if(recurso != null) {
					...
					rec.setDataFim(fieldLogisticaDataFim.getDate());
					rec.setDataInicio(fieldLogisticaDataInicio.getDate());
					try {
					...
					JOptionPane.showMessageDialog(FormOrdemServicoExtra.this, "Selecione um recurso!", "ERRO", JOptionPane.ERROR_MESSAGE);
				}
			}
		});
		...
		addCenografiaButton.addActionListener(new java.awt.event.ActionListener() {
			public void actionPerformed(java.awt.event.ActionEvent e) {
				if(recurso != null) {
					RecursoSolicitado rec = new RecursoSolicitado();
					rec.setRecurso(recurso);
					rec.setDataFim(fieldCenografiaDataFim.getDate());
					rec.setDataInicio(fieldCenografiaDataInicio.getDate());
					try {
	...
	private JPanel getPanelImportarOrcamento() {
		...
		pOrcamento.add(getBotaoProcurarOS() );
	...
	public void loadForm2(OrdemServico rec, boolean travarBotaoAdicionar) {
		...
		setTitle("Ordem de Serviço Extra - NÚMERO DA OS: " + rec.getId() + " (" + rec.getNomeEvento() + ")");
	...
	private JButton getBotaoImportarOrcamento() {
		...
		botaoOrcamento.addActionListener(
				new ActionListener() {
					public void actionPerformed(ActionEvent arg0) {
						DialogSearchOrcamento3 teste = new DialogSearchOrcamento3(FormOrdemServicoExtra.this);
						Orcamento o = teste.showDialog(FormOrdemServicoExtra.this);
						if(o != null) {
		...
		addTerceirizadoButton.addActionListener(new java.awt.event.ActionListener() {
			public void actionPerformed(java.awt.event.ActionEvent e) {
				if(recursoTerceirizado != null) {
					...
					rec.setDataFim(fieldTerceirizadoDataFim.getDate());
					rec.setDataInicio(fieldTerceirizadoDataInicio.getDate());
					try {
		...
		addRecursoButton.addActionListener(new java.awt.event.ActionListener() {
			public void actionPerformed(java.awt.event.ActionEvent e) {
				if(recurso != null) {
					RecursoSolicitado rec = new RecursoSolicitado();
					rec.setRecurso(recurso);
					rec.setDataFim(fieldEquipamentoDataFim.getDate());
					rec.setDataInicio(fieldEquipamentoDataInicio.getDate());
					try {
		...
		addEquipeTecnicaButton.addActionListener(new java.awt.event.ActionListener() {
			public void actionPerformed(java.awt.event.ActionEvent e) {
				if(recurso != null) {
					RecursoSolicitado rec = new RecursoSolicitado();
					rec.setRecurso(recurso);
					rec.setDataFim(fieldEquipeTecnicaDataFim.getDate());
					rec.setDataInicio(fieldEquipeTecnicaDataInicio.getDate());
					try {
	...
	public void carregarOS(OrdemServico c) {
		osOriginal = c;
		if(c!=null){
			...
			fieldNomeContatoEvento.setText(c.getContatoEvento());
			fieldTelefoneContatoEvento.setText(c.getTelefoneContatoEvento());
			fieldFuncionario.setSelectedItem(c.getFuncionario());
			fieldStatusOS.setSelectedItem(c.getStatus());
			...
			carregarLocal(c.getLocal().getLocal());
			abrirPlanilha(c.getPlanilha());
	...
	private void carregarLocal(Local l) {
		if(l!=null){
			...
			fieldLocalSalvar.setSelected(false);
		} else {
			limparLocal();
		}
	}
	...
	public JButton getBotaoProcurarCliente() {
		...
		botaoProcurarCliente.addActionListener(
				new ActionListener() {
					public void actionPerformed(ActionEvent arg0) {
						DialogSearchClient teste = new DialogSearchClient(FormOrdemServicoExtra.this);
						Cliente c = teste.showDialog(FormOrdemServicoExtra.this);
						if(c != null)
							carregarCliente(c);
					}
				}	
	...
	public JButton getBotaoProcurarOS() {
		JButton botaoProcurarOS = new JButton("Importar OS", new ImageIcon(getClass().getResource("/images/find.png")));
		botaoProcurarOS.addActionListener(
				new ActionListener() {
					public void actionPerformed(ActionEvent arg0) {
						DialogSearchOS teste = new DialogSearchOS(FormOrdemServicoExtra.this);
						OrdemServico c = teste.showDialog(FormOrdemServicoExtra.this);
						if(c != null)
							carregarOS(c);
					}
				}	
		);
		return botaoProcurarOS;
	}
				...
				new ActionListener() {
					public void actionPerformed(ActionEvent arg0) {
						if(cliente != null) {
							...
							JOptionPane.showMessageDialog(FormOrdemServicoExtra.this, "Nenhum cliente selecionado!", "ERRO", JOptionPane.ERROR_MESSAGE);
						}
	...
	public JButton getBotaoProcurarRecurso() {
		...
		botaoProcurarRecurso.addActionListener(
				new ActionListener() {
					public void actionPerformed(ActionEvent arg0) {
						DialogSearchRecurso teste = new DialogSearchRecurso(FormOrdemServicoExtra.this, Facade.getInstance().listarRecursos());
						Recurso r = teste.showDialog(FormOrdemServicoExtra.this);
						if(r != null)
							carregarRecurso(r);
					}
				}	
	...
	public JButton getBotaoProcurarLocal() {
		...
		botaoProcurarLocal.addActionListener(
				new ActionListener() {
					public void actionPerformed(ActionEvent arg0) {
						DialogSearchLocal teste = new DialogSearchLocal(FormOrdemServicoExtra.this, Facade.getInstance().listarLocais());
						Local l = teste.showDialog(FormOrdemServicoExtra.this);
						if(l != null) {
							carregarLocal(l);
							carregarFieldAmbientesEvento(l);
						}
	...
	protected void clear() {
		...
		setTitle("Ordem de Serviço Extra");
	}
	...
	protected boolean validateFormInsert() {
		...
		if(ordemServ != null) {
			error += "\nNão é possível duplicar uma Ordem de Serviço.";
			test = false;
		}
		...
		if(cliente != null) {
			...
			Cliente cl2 = Facade.getInstance().carregarCliente(cliente.getId());
			if(ordemServ.getStatus() != null) {
				if(cl2.getStatus() != StatusCliente.ATIVO && ordemServ.getStatus() == StatusOS.PENDENTE) {
					error += "\nNão é possível aprovar a realização desta OS, \\npois o cliente não se encontra no status ATIVO. \\nAtualize o cadastro de cliente antes de prosseguir.";
					test = false;
				}
			}
	...
	public List<OrdemServico> listAll() {
		...
		List<OrdemServico> listaExtras = new ArrayList<OrdemServico>();
